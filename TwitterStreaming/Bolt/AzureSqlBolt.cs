...
namespace TwitterSentimentAnalysis.Bolt
{
    ...
    public class AzureSqlBolt : ISCPBolt
    {
        ...
        public AzureSqlBolt(Context context, Dictionary<string, Object> parms)
        {
            ...
            inputSchema.Add("TWEETRANK_STREAM", TweetRankBolt.OutputSchema);
        ...
        {
            Context.Logger.Info("SQL AZURE: Insert method called");
            PrepareSqlInsertCommand(row);
            for (int i = 0; i < this.SqlTableColumns.Count; i++)
            {
                this.SqlInsertCommand.Parameters["@" + this.SqlTableColumns[i]].Value = row[i];
            }
            Context.Logger.Info("SQL AZURE: Insert method end:" + this.SqlInsertCommand);
            ...
            try
            {
                ...
                using (var sr = new StreamReader(response.GetResponseStream()))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Context.Logger.Error("An error occured while executing GetSentimentType: {0}. Exception Details:\r\n{1}",
                   textToAnalyze, ex.ToString());
                        return 4;
                    }
                }
            } catch (Exception ex)
